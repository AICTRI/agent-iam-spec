...
| `audit.openapi.json` | 6 | Security-event ingestion API (OpenAPI 3.1) |

Reusable request body schemas are under `requests/`. Field-to-clause mappings are in `FIELD-MAPPING.md`.

## Fixtures

Valid and expected-invalid record and request instances are under `fixtures/`;
`fixtures/manifest.json` maps each instance to its schema. Run
`node ../conformance/validate.mjs` to validate the full fixture set.
